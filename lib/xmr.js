...
function blobTypeKCN(blob_type_num) {
    return blob_type_num == 105;
}
...
function convertBlob(blobBuffer, blob_type_num){
    ...
    else if (blobTypeKCN(blob_type_num)) return cnUtil.convertKcnBlob(blobBuffer);
...
function constructNewBlob(blockTemplate, NonceBuffer, blob_type_num, ring){
    ...
    } else if (blobTypeKCN(blob_type_num)) {
        return cnUtil.constructNewKcnBlob(blockTemplate, NonceBuffer);
    } else {
...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cryptonote_sal':  return 15; // SAL
        case 'cryptonote_xeq':  return 22; // XEQ
        case 'cryptonote_dero': return 100; // Dero
        case 'raptoreum':       return 104; // RTM
        case 'raptoreum_kcn':   return 105; // KCN
    }
    return 0;
}
