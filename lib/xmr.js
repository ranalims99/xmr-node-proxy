...
function parse_blob_type(blob_type_str) {
    ...
    switch (blob_type_str) {
        ...
        case 'cryptonote_qrl': return 9; // Qrl
    }
    return 0;
}
...
function BlockTemplate(template) {
    ...
    this.previousHash = Buffer.from(Array(32));
    this.workerNonce = 0;
    this.solo = false;
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    let template = Buffer.from(blockTemplate.buffer);
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        if (++ verified_share_num <= MAX_VER_SHARES_PER_SEC*VER_SHARES_PERIOD) {
            ...
            let convertedBlob;
            switch (blockTemplate.blob_type) {
            case 9: // QRL
                Buffer.from(nonce, 'hex').copy(template, 39);
                convertedBlob = template;
                break;
            default:
                convertedBlob = cnUtil.convert_blob(
                    cnUtil.construct_block_blob(template, Buffer.from(nonce, 'hex'), blockTemplate.blob_type),
                    blockTemplate.blob_type
                );
